...
%-------------------------------------------------------------------------%
% NOTE: This is a customized version of PolyMesher. There are some        % 
...
function [Node,Element,Supp,Load,P] = ...
                     PolyMesher_with_crack(Domain,NElem,MaxIter,MeshFile,crack_tips_coords,pltoptions,P)
...
while(It<=MaxIter && Err>Tol)
  ...
  %if NElem<=6000, PolyMshr_PlotMsh(Node,Element,NElem,crack_tips_coords,pltoptions); end
end
...
PolyMshr_PlotMsh(Node,Element,NElem,crack_tips_coords,pltoptions,Supp,Load);         %Plot mesh and BCs
% A.O-B
BoundaryNodes=Domain('Boundary',{Node});
EnrichedNodes=input('Enter enriched nodes in the format [...; node_number enrichment_type; ...] (enrichment_type: 1 (Heaviside) or 2 (crack-tip))\n');
PolyMesher2VEMLab_with_crack(Node,Element,NElem,...      %Plot mesh to a VEMLab mesh format
                             BoundaryNodes,MeshFile,DomainType,BdBox,EnrichedNodes); 
...
%---------------------------------------------------------------- PLOT MESH
function PolyMshr_PlotMsh(Node,Element,NElem,crack_tips_coords,pltoptions,Supp,Load)
...
if pltoptions.plot_node_numbers
  mesh_size = min_edge_size(Node,Element,NElem);
  offset_x = 0.06*mesh_size; offset_y = 0.08*mesh_size;
  for i = 1:numnd
    idstring = sprintf('%d',i);
    text(Node(i,1)+offset_x, Node(i,2)+offset_y, idstring,'color',[1,0,0],'fontsize',pltoptions.fsize);
  end
end
% element numbering
