function create_cookmembrane_gunelve_mesh
  close all;
  clc;
  ...
  npattern_x=5; % number of patterns along x coordinate
  npattern_y=5; % number of patterns along y coordinate
  mesh_filename='cook_membrane_gunelvemesh_5x5.txt'; 
  ...
  % make Cook's membrane geometry by mapping from the unit square: must go
  % after BoundaryNodes search
  coords=[48*coords(:,1),44*coords(:,1)+44*coords(:,2)-28*coords(:,1).*coords(:,2)];   
