...
%-------------------------------------------------------------------------------
%
%                     2D Plate With a Hole in Tension
...
function plate_with_hole_linelast2d
...
%   mesh_filename='plate_with_hole_600poly_elems.txt';
  ...
  % add Dirichlet boundary nodes and dofs of the left boundary
  Dirichlet_boundary_nodes=domainMesh.boundary_nodes.Left;
  Dirichlet_boundary_dofs=domainMesh.boundary_dofs.Left; 
  % add middle node of right boundary to the Dirichlet boundary nodes and dofs
  Dirichlet_boundary_nodes=[Dirichlet_boundary_nodes;domainMesh.boundary_nodes.MidNodeRightFace];
  Dirichlet_boundary_dofs=[Dirichlet_boundary_dofs;domainMesh.boundary_dofs.MidNodeRightFace];
  % define the values of the Dirichlet boundary conditions
...
%% DEFINITION OF THE BODY FORCE FUNCTIONS FOR THE PLATE WITH A HOLE
...
%% DEFINITION OF NEUMANN FUNCTIONS FOR THE PLATE WITH A HOLE
% to apply on the right boundary
...
%% DEFINITION OF DIRICHLET FUNCTIONS FOR THE PLATE WITH A HOLE
% to apply on the left boundary and on the middle node of the righ boundary
function ux = ux_Dirichlet_fun(x,y,matProps)
  ...
  ux=zeros(length(x),2); % first column = value; second column = free (1) or fixed (0)
                         ...
                         % values of ux(:,1)
  % free the ux dof of the middle node of the right boundary (was the last one
  % added in the list of boundary nodes, so it is the last one in the ux array)
  ux(length(x),2)=1;
end
function uy = uy_Dirichlet_fun(x,y,matProps)
  ...
  uy=ones(length(y),2);  % first column = value; second column = free (1) or fixed (0)
                         % In this case, all uy dofs are free and thus the
                         % values in uy(:,1) will be ignored.   
  % fix the uy dof of the middle node of the right boundary (was the last one
  % added in the list of boundary nodes, so it is the last one in the uy array)
  uy(length(y),2)=0; 
  uy(length(y),1)=0; % update the value
end
