...
% Other References 
% ================
% [2] https://la.mathworks.com/help/pde/ug/solve-poissons-equation-on-a-unit-disk.html
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function unit_disk_poisson2d_norms
...
%   mesh_filename={'unit_disk_poisson2d_111poly_elems_random_ssalinas.txt',...
%                  'unit_disk_poisson2d_667poly_elems_random_ssalinas.txt',...
%                  'unit_disk_poisson2d_5225poly_elems_random_ssalinas.txt',...
%                  'unit_disk_poisson2d_30655poly_elems_random_ssalinas.txt'};      

  mesh_filename={'unit_disk_poisson2d_108poly_elems_semiuniform_ssalinas.txt',...
                 'unit_disk_poisson2d_993poly_elems_semiuniform_ssalinas.txt',...
                 'unit_disk_poisson2d_5070poly_elems_semiuniform_ssalinas.txt',...
                 'unit_disk_poisson2d_25257poly_elems_semiuniform_ssalinas.txt'}; 
  ...
  for i=1:length(mesh_filename)      
    [h_max_1,L2rel_1,H1rel_1]=...
      unit_disk_poisson2d(opsystem,vemlab_root_dir,mesh_filename{i},'VEM2D');
    xcm(1,i)=h_max_1;
    ycm(1,i)=H1rel_1;
    ycm1(1,i)=L2rel_1;      
  end
  textnote='semiuniform'; %'random'; %'semiuniform';%'$\lambda/\mu=5\times 10^7$';
...
function H1plot(xcm,ycm,textnote)
  ...
  scale = 0.025;  % to scale the triangle
  x_shift_bottom = 3.0;  % to center in the x-direction the number that goes on the bottom side of the triangle
  y_shift_bottom = 0.2; % to center in the y-direction the number that goes on the bottom side of the triangle
...
%   legend1 = legend(s1,'Interpreter','latex','location','southeast');  
...
function L2plot(xcm,ycm,textnote)
  ...
  scale = 0.025;  % to scale the triangle
  x_shift_bottom = 3.0;  % to center in the x-direction the number that goes on the bottom side of the triangle
  y_shift_bottom = 0.30; % to center in the y-direction the number that goes on the bottom side of the triangle
  x_shift_right = 0.06;  % to center in the x-direction the number that goes on the right side of the triangle
...
function [h_max,L2rel,H1rel]=unit_disk_poisson2d(opsystem,vemlab_root_dir,mesh_filename,vemlab_method)
...
function b = source_term_fun(x,y,matProps)
  ...
  b=1.0;  % is used as a force per volume
end
