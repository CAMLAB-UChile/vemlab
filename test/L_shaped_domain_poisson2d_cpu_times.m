...
function L_shaped_domain_poisson2d_cpu_times
  ...
  for n=1:nrep
    ...
    for i=1:length(mesh_filename_voronoi_semiuniform)      
      ...
      xcm(4,i)=ndofs;
      ycm(4,i)=ycm(4,i)+cpu_time;      
    end
  end
  ycm=(1/nrep)*ycm; % average ycm
  max_cpu_time=max([ycm(1,:), ycm(2,:), ycm(3,:), ycm(4,:)]);
  ycm=(1/max_cpu_time)*ycm; % normalized CPU time
   
  cpu_plot(xcm,ycm); % plot normalized CPU time vs DOF
                                     
end

function cpu_plot(xcm,ycm)
  ...
  ylabel('Normalized CPU time','interpreter','latex','fontsize', fsize);  

  axis([10^2, 10^6, 10^-3.5, 10^0.5]);
  ...
  legend1 = legend(s1,s2,s3,s4,'location','southeast');    
  set(legend1,'FontSize',fsize-2);

end

function [cpu_time,ndofs]=L_shaped_domain_poisson2d(opsystem,vemlab_root_dir,mesh_filename,vemlab_method)
