function patch_plot_figure(config,entity,polygons,nodescoords,fieldvar,...
                           fieldvartitleclb,fieldvartitlefigfile,clb_limits,dxtitle,soltype)

  % entity: 'on_faces' or 'on_nodes' (for VEM or FEM only since patch plot for NIVED is performed on nodes only
...
%   figure('InvertHardcopy','off','PaperType','<custom>','PaperSize',[5.83 4.38],'Color',[1 1 1],'Renderer','opengl');
  figure;
  ...
  if strcmp(config.vemlab_method,'VEM2D')||strcmp(config.vemlab_method,'FEM2DT3')||strcmp(config.vemlab_method,'FEM2DQ4')
    if strcmp(config.plot_mesh_over_results,'yes')
      if strcmp(entity,'on_faces')
        patch('Faces',elements,'Vertices',nodescoords,...
              'FaceColor','flat','FaceVertexCData',fieldvar,'LineWidth',config.plot_mesh_linewidth);  
      elseif strcmp(entity,'on_nodes')
        patch('Faces',elements,'Vertices',[nodescoords,fieldvar],...
              'FaceColor','interp','CData',fieldvar,'LineWidth',config.plot_mesh_linewidth);          
      else
        throw_error('In patch_plot_figure.m: entity');
      end
    else
      if strcmp(entity,'on_faces')
        patch('Faces',elements,'Vertices',nodescoords,...
              'FaceColor','flat','EdgeColor','none','FaceVertexCData',fieldvar);
      elseif strcmp(entity,'on_nodes')
        patch('Faces',elements,'Vertices',[nodescoords,fieldvar],...
              'FaceColor','interp','EdgeColor','interp','CData',fieldvar);        
      else
        throw_error('In patch_plot_figure.m: entity');
      end
    end
  else
    throw_error('In plot_patch_figure.m: vemlab_method');
  end  
...
%     clb.Title.Position(1:2) = [dxtitle*clb.Title.Position(1) 1.0*clb.Title.Position(2)];
    ...
    elseif (length(clb_limits)==1)||(length(clb_limits)>2)
      throw_warning('In patch_plot_figure.m: clb_limits must be a two-column row. Using default colorbar limits'); 
...
%     set(gcf,'InvertHardcopy','off','PaperType','<custom>','PaperSize',[5.83 4.38],'Color',[1 1 1],'Renderer','painters');  
    ...
    throw_error('In patch_plot_figure.m: config.printer_renderer');
  end
...
%       else
  ...
  elseif strcmp(soltype,'exact')
    ...
    throw_error('In patch_plot_figure.m: solutiontype');
  end
