function [K_global,f_global] = vem_assembly_linelast2d(domainMesh,matProps,body_force_fun_values,config)
  ...
  for e=1:numel 
    ...
    ind(2*range)=2*nodes;
    K_global(ind,ind)=K_global(ind,ind)+vem_stiffness_linelast2d(verts,matProps,config);
    f_global(ind)=f_global(ind)+vem_body_force_linelast2d(verts,body_force_fun_values,matProps);      
  end
end
